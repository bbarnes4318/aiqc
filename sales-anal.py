...
def analyze_transcript_with_chatgpt(transcript, call_type):
    """
    Analyzes the call transcript using ChatGPT-4 to determine various aspects of the call.
    """
    prompt = f"""
    You are an expert sales data analyst. You excel at identifying details in sales conversions that either lead to a sale or lead away from a sale. Your task is to analyze the following call transcript:

    {transcript}

    Based on the transcript, determine:
    1. Common phrases, questions, and responses.
    2. The tone, sentiment, and key points.
    3. Recurring objections and how they were handled.
    4. Patterns in conversation flow.

    Specifically:
    - If it's a {call_type} call, identify details that led to this outcome.
    - Compare patterns between successful and unsuccessful calls.
    - Offer insights and recommendations for improving sales techniques.
    """
    
    response = openai.ChatCompletion.create(
        model="gpt-4-0125-preview",
        messages=[
            {"role": "system", "content": "You are a highly intelligent AI trained to analyze call transcripts for sales and customer service purposes."},
...
def process_call(url, call_type):
    ...
    try:
        ...
        analysis_result = analyze_transcript_with_chatgpt(transcript, call_type)
    finally:
...
def process_all_calls():
    ...
    for i, url in enumerate(urls):
        call_type = 'successful' if i < 10 else 'unsuccessful'
        print(f"Processing: {url} ({call_type} call)")
        result = process_call(url, call_type)
